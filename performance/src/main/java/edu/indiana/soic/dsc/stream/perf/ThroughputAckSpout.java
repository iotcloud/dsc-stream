...
//      String id = String.valueOf(totalSendCount);
  ...
  public void ack(Object o) {
    if ((debug && ackReceiveCount % printInveral == 0) || startFailing) {
      LOG.error("Acked tuple: " + o.toString() + " total acked: " + totalAckCount + " send: " + totalSendCount + " faile: " + totalFailCount);
    }
    totalAckCount++;
    handleAck(false, 0);
  }
  ...
  public void fail(Object o) {
    LOG.error("Failed to process tuple: " + o.toString() + " total acked: " + totalAckCount + " send: " + totalSendCount + " faile: " + totalFailCount);
    startFailing = true;
    totalFailCount++;
    handleAck(true, o);
  }
  ...
  public void handleAck(boolean fail, Object ack) {
    ...
    if (sendState == SendingType.EMPTY) {
      ...
      if (currentSendCount >= noOfMessages && ackReceiveCount >= noOfMessages) {
        ...
        String currentOutPut = size + " " + noOfMessages + " " + time + " " + (noOfMessages + 0.0) / (time / 1000.0);
